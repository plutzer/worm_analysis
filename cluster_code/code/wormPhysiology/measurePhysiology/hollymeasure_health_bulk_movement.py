...
import os
import time
import wormPhysiology.measurePhysiology.extractFeatures_bulk_movement as extractFeatures
...
class HollyMeasurer(od.WormMeasurer):
	def __init__(self, worm_subdirectory, working_directory, validated_directory):
		...
		self.worm_frame = pd.DataFrame(index = self.worm_times, columns = [
			...
			'unstimulated_rate', 'bulk_movement', 
	...
	def make_measurements(self, my_time, last_mask, focal_mask, movement_masks,temporal_radius,i, last_frame, focal_frame, eggs_mask):
		...
		if my_time == self.timepoints[temporal_radius]:
			(self.worm_frame.loc[my_time], bulk_move_colored) = extractFeatures.measure_movementCOM(movement_masks, [focal_mask, focal_mask], [focal_frame, focal_frame], self.position_metadata[i], self.worm_frame.loc[my_time]) 
		else:
			(self.worm_frame.loc[my_time], bulk_move_colored) = extractFeatures.measure_movementCOM(movement_masks, [last_mask, focal_mask], [last_frame, focal_frame], self.position_metadata[i], self.worm_frame.loc[my_time]) 
