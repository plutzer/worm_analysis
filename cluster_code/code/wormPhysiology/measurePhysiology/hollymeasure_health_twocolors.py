...
class HollyMeasurer(od.WormMeasurer):
	...
	def make_measurements(self, my_time, last_mask, focal_mask, movement_masks,temporal_radius,i, last_frame, focal_frame, eggs_mask):
		...
		if os.path.isfile(self.worm_subdirectory + os.path.sep + my_time + ' ' + 'green_yellow_excitation_autofluorescence.png'):
			...
			(self.worm_frame.loc[my_time], rfp_fluorescence_colored) = self.measure_rfp_fluorescence(focal_fluorescence, focal_mask, self.worm_frame.loc[my_time])
		else:
		...
		return (last_frame, last_mask, focal_fluorescence, worm_eggs_colored, rfp_fluorescence_colored, movement_colored, bulk_move_colored, gfp_fluorescence_colored,gfp_focal_fluorescence)
	...
	def measure_rfp_fluorescence(self,fluorescent_image, worm_mask, time_series):
		...
		time_series.loc[['integrated_rfp', 'median_rfp', 'percentile95_rfp', 'expressionarea_rfp', 'expressionareafraction_rfp', 'expressionmean_rfp', 
		'highexpressionarea_rfp', 'highexpressionareafraction_rfp', 'highexpressionmean_rfp', 'highexpressionintegrated_rfp','percentile99_rfp', 'max_rfp','integrated99_rfp','percentile99area_rfp']] = (integrated, median, percentile95, expression_area, expression_area_fraction, expression_mean, high_expression_area, high_expression_area_fraction, high_expression_mean, high_expression_integrated, percentile99,maximum,integrated99, percentile99area)
